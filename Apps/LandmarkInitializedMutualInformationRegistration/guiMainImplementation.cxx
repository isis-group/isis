/*=========================================================================
  ...
  Module:    $RCSfile: guiMainImplementation.cxx,v $
  Language:  C++
  Date:      $Date: 2007-04-30 14:58:48 $
...
::HandleInputImageChange(std::string & fileName, bool isFixedImage)
  ...
  if ( isFixedImage )
    ...
    ImageType::PointType originFixed;
    ImageType::PointType originMoving;
    ImageType::PointType originFixedtmp;
    ImageType::PointType originMovingtmp;

    ImageType::SpacingType spacingFixed;
    ImageType::SpacingType spacingMoving;

	
	//gathering infomration from nifti-file
    double qoffsetFixed[3], qoffsetMoving[3];
    nifti_1_header *headerFixed, *headerMoving;
    headerFixed = nifti_read_header(m_FixedImageFileName.c_str(), NULL, 1);
    headerMoving = nifti_read_header(m_MovingImageFileName.c_str(), NULL, 1);

    disp_nifti_1_header(NULL, headerFixed);
    disp_nifti_1_header(NULL, headerMoving);

    qoffsetFixed[0] = headerFixed->qoffset_x;
    qoffsetFixed[1] = headerFixed->qoffset_y;
    qoffsetFixed[2] = headerFixed->qoffset_z;

    qoffsetMoving[0] = headerMoving->qoffset_x;
    qoffsetMoving[1] = headerMoving->qoffset_y;
    qoffsetMoving[2] = headerMoving->qoffset_z;
    
    tkMovingImageFile->value(m_LastLoadedImagePath.c_str());
    m_MovingImage = image;//.GetPointer();
	originMoving = m_MovingImage->GetOrigin();
	ImageType::SizeType sizeFixed = m_FixedImage->GetLargestPossibleRegion().GetSize();
	ImageType::SizeType sizeMoving = m_MovingImage->GetLargestPossibleRegion().GetSize();
	
	originFixedtmp = m_FixedImage->GetOrigin();
    originMovingtmp = m_MovingImage->GetOrigin();
	/*
	originMoving[0] = originFixedtmp[0] + qoffsetFixed[0];
	originMoving[1] = originFixedtmp[1] + qoffsetFixed[1];
	originMoving[2] = originFixedtmp[2] + qoffsetFixed[2];

	originFixed[0] = originMovingtmp[0] + qoffsetMoving[0];
	originFixed[1] = originMovingtmp[1] + qoffsetMoving[1];
	originFixed[2] = originMovingtmp[2] + qoffsetMoving[2];
*/
    //m_MovingImage->SetOrigin( m_FixedImage->GetOrigin() );
    //m_MovingImage->SetOrigin( originMoving );
	//m_FixedImage->SetOrigin( originFixed );
	//m_MovingImage->SetSpacing( m_FixedImage->GetSpacing() );
    
	spacingFixed = m_FixedImage->GetSpacing();
    spacingMoving = m_MovingImage->GetSpacing();

    std::cout << "size fixed image: " << sizeFixed[0] << "  " << sizeFixed[1] << "  " << sizeFixed[2] << std::endl;
    std::cout << "size moving image: " << sizeMoving[0] << "  " << sizeMoving[1] << "  " << sizeMoving[2] << std::endl;

	
    std::cout << "spacing fixed image: " << spacingFixed[0] << "  " << spacingFixed[1] << "  " << spacingFixed[2] << std::endl;
    std::cout << "spacing moving image: " << spacingMoving[0] << "  " << spacingMoving[1] << "  " << spacingMoving[2] << std::endl;
	
    originFixed = m_FixedImage->GetOrigin();
    originMoving = m_MovingImage->GetOrigin();
   
    std::cout << "origin fixed image: " << originFixed[0] << "  " << originFixed[1] << "  " << originFixed[2] << std::endl;
    std::cout << "origin moving image: " << originMoving[0] << "  " << originMoving[1] << "  " << originMoving[2] << std::endl;
...
::LoadImage( const char * filename )
  ...
  //use orientimagefilter to use the correct orientation stored in the header
