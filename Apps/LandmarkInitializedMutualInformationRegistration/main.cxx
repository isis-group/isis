...
int main(int argc, char **argv)
  ...
  if(argc > 1)
    ...
    typedef itk::OrientImageFilter<ImageType,ImageType> OrientImageFilterTypeFixed;
    // force image into known orientation
    OrientImageFilterTypeFixed::Pointer orienterFixed =OrientImageFilterTypeFixed::New();
    orienterFixed->SetInput(fixedReader->GetOutput());
    orienterFixed->SetUseImageDirection(true);

    try
      {
      orienterFixed->Update();
      }
    catch(...)
      {
    ...
    imageRegistrationApp->SetFixedImage( orienterFixed->GetOutput() );
    ...
    typedef itk::OrientImageFilter<ImageType,ImageType> OrientImageFilterTypeMoving;
    // force image into known orientation
    OrientImageFilterTypeMoving::Pointer orienterMoving =OrientImageFilterTypeMoving::New();
    orienterMoving->SetInput(movingReader->GetOutput());
    orienterMoving->SetUseImageDirection(true);
    try
      {
    ...
    ImageType::Pointer imageFixed = orienterFixed->GetOutput();
    ImageType::Pointer imageMoving = orienterMoving->GetOutput();
    imageMoving->SetOrigin( imageFixed->GetOrigin() );
    imageRegistrationApp->SetMovingImage( orienterMoving->GetOutput() );


    //print the origin
   
    imageRegistrationApp->SetFixedImageRegion( 
                                orienterFixed->GetOutput()
