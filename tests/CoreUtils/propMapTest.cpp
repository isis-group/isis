...
BOOST_AUTO_TEST_CASE( propMap_init_test )
{
	...
	BOOST_CHECK( not ref.empty());
	BOOST_CHECK_EQUAL( ref.propertyValue("Test1"), 1 );
	BOOST_CHECK_EQUAL( ref.propertyValue("Test2"), 2 );
...
BOOST_AUTO_TEST_CASE( propMap_remove_test )
{
	...
	BOOST_CHECK( not map.branch("sub").empty() ); //the submap must still be there
...
BOOST_AUTO_TEST_CASE( propMap_transform_test )
{
	...
	BOOST_CHECK_EQUAL( map.propertyValue("sub/Test1float"), 1 );// this will do an automatic transform back to int for comparison
	BOOST_CHECK_EQUAL( map.propertyValue("Test1int"), 6 );
