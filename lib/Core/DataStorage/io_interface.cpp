...
bool FileFormat::hasOrTell( const util::PropMap::KeyType &name, const isis::util::PropMap &object, isis::LogLevel level )
{
...
std::string FileFormat::makeFilename(const util::PropMap &props,std::string namePattern)
{
	...
	{
		const util::PropMap::KeyType prop(what[0].str().substr(1,what.length()-2).c_str());
		if(props.hasProperty(prop)){
			namePattern.replace(what[0].first,what[0].second,props.getPropertyAs<std::string>(prop));
			LOG(Debug,info)
			<< "Replacing " << util::MSubject(util::PropMap::KeyType("{")+prop+"}") << " by "	<< util::MSubject( props.getPropertyAs<std::string>(prop) )
