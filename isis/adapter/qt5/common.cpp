...
{
	auto transfer_function = [scaling](uchar *dst, size_t line_length, const data::ValueArrayBase &line){
		line.copyToMem<uint8_t>(dst,line_length,scaling);
	};
	return makeQImage(data,line_length,transfer_function);
}

QImage isis::qt5::makeQImage(const data::ValueArrayBase& data, size_t line_length, const std::function<void (uchar *, size_t, const data::ValueArrayBase &)> &transfer_function)
{
	...
	for(auto line:data.splice(line_length)) {
		...
		transfer_function(dst,line_length,*line);
		++line_idx;
	}
	return ret;
}
