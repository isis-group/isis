...
data::Image isis::itk4::rotate(data::Image src, std::pair<int,int> rotation_plain, float angle)
{
	switch ( src.getMajorTypeID() ) {
	case data::ValueArray<int8_t>::staticID():
		return rotate_impl<int8_t>( src, rotation_plain, angle);
		break;
	case data::ValueArray<uint8_t>::staticID():
		return rotate_impl<uint8_t>( src, rotation_plain, angle);
		break;
	case data::ValueArray<int16_t>::staticID():
		return rotate_impl<int16_t>( src, rotation_plain, angle);
		break;
	case data::ValueArray<uint16_t>::staticID():
		return rotate_impl<uint16_t>( src, rotation_plain, angle);
		break;
	case data::ValueArray<int32_t>::staticID():
		return rotate_impl<int32_t>( src, rotation_plain, angle);
		break;
	case data::ValueArray<uint32_t>::staticID():
		return rotate_impl<uint32_t>( src, rotation_plain, angle);
		break;
	case data::ValueArray<float>::staticID():
		return rotate_impl<float>( src, rotation_plain, angle);
		break;
	case data::ValueArray<double>::staticID():
	default:
		return rotate_impl<double>( src, rotation_plain, angle);
		break;
	}
}
